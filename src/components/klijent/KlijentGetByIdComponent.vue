<template>
  <div class="d-flex justify-content-center" style="height: 100%">
    <div class="card bg-dark mb-3" v-if="klijent">
      ...
      <div class="card-body">
        <h5 class="card-title text-warning">{{ klijent.ime }} {{ klijent.prezime }}</h5>
        <p class="card-text text-white">Kontakt: {{ klijent.kontakt }}</p>
        <p class="card-text text-white">ID: {{ klijent.id }}</p>
        <a href="#" @click="showEdit= true" class="btn btn-warning mr-5">Izmeni</a>
        <a href="#" @click="showDelete= true" class="btn btn-danger">Obrisi</a>
      </div>
      <div v-if="showEdit">
        <KlijentEditModal :showMe="showEdit" :klijent="klijent" @closeModal="hideModalEdit()" />
      </div>
      <div v-if="showDelete">
        <KlijentDeleteModal
          :showMe="showDelete"
